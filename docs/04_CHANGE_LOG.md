...
## 2026-07-14 (4) — Pig Pig Chat: thu gọn Suggested Questions và cân bằng Chat Input/Send Button

### Nguồn gốc

Merge trực tiếp từ requirement mới của Dương theo quy trình chính thức đã ghi tại `docs/00_TECH_RULES.md`: viết spec chính xác, merge vào spec chính thức, cập nhật Delta/Change Log, sau đó sửa implementation trong cùng change set.

### Lý do

- Bản deploy đang render toàn bộ 20 Suggested Questions/FAQ ngay khi mở Pig Pig Chat, làm nội dung quá dài.
- Chat Input hiển thị Label "Message", khiến Send Button nhìn cao/lệch so với khung nhập.

### Spec thay đổi

- `feature-specs/27_PIG_PIG.md` — xác nhận Version 1.1, Status Final đã chứa đúng requirement: mặc định 03 câu, View all mở tối đa 20 câu, View less thu gọn, không hiển thị "Message", Input và Send Button cùng chiều cao.
- `specs/17_UI_LAYOUT.md`:
  - Mô tả rõ trạng thái Collapsed/Expanded của Suggested Questions.
  - Thêm interaction rules và acceptance expectations cho View all/View less.
  - Quy định Chat Input và Send Button cùng chiều cao 56 px.
  - Bỏ visible Label "Message".
  - Quy định Error Message nằm dưới Input và không làm lệch mép trên của Send Button.
- `docs/03_DELTA_SPEC.md` — thêm `DELTA-008` với trạng thái Merged.

### Code thay đổi

- `src/features/pig-pig/components/pig-pig-screen.tsx`:
  - `SuggestionChips` mặc định chỉ render 03 câu.
  - Thêm Text Button "View all"; khi mở rộng đổi thành "View less".
  - Giữ nguyên tối đa 20 câu hỏi hiện có.
  - Bỏ import và usage của `Input` component tại Chat Input.
  - Thay bằng native input có `aria-label`, giữ Placeholder hiện tại.
  - Input cao 56 px; Send Button 56 × 56 px và thẳng hàng.
  - Giữ nguyên Enter-to-send, validation tin nhắn trống, disabled state và Chat History.

### Không thay đổi

- Data Model và Local Storage schema.
...
## 2026-07-16 — Category Analysis: đổi từ Pie Chart sang Donut Chart + Tổng chi tiêu ở trên + Legend đầy đủ chiều rộng

### Nguồn gốc

Requirement mới từ Dương (kèm ảnh tham khảo thiết kế), theo quy trình chính thức tại `docs/00_TECH_RULES.md`: viết spec chính xác, merge thẳng vào spec chính thức, cập nhật Change Log, sau đó sửa code — không dừng lại chờ duyệt trước khi gửi.

### Lý do

Bản hiện tại (Pie Chart đặc kín, Legend đặt cạnh biểu đồ, Amount chỉ hiện khi tương tác vào slice) chưa đúng cảm giác trực quan mong muốn. Dương gửi ảnh tham khảo: biểu đồ dạng vành khuyên (donut) dày, Tổng chi tiêu hiển thị to phía trên biểu đồ, Legend dạng danh sách đầy đủ chiều rộng bên dưới, mỗi dòng hiển thị rõ tên Category, số tiền và phần trăm.

### Spec thay đổi

- `feature-specs/25_REPORT.md` — Version 1.2 → 1.3. Section 2 "Category Analysis": đổi mô tả từ Pie Chart sang Donut Chart, thêm quy tắc hiển thị Total Spending phía trên biểu đồ, đổi Legend sang dạng danh sách đầy đủ chiều rộng với Amount luôn hiển thị mặc định (không còn ẩn/chờ tương tác slice). Cập nhật Section 10 (Screen Content), Section 14 (Display Rules), Section 18 (Related Specification — CMP-017), AC-004; thêm AC-017 (không hiển thị badge so sánh kỳ trước, ngoài phạm vi MVP).
- `specs/16_COMPONENT_LIBRARY.md` — Version 1.0 → 1.1. Cập nhật CMP-017: đổi tên thành "Category Pie Chart (Donut Chart)", viết lại Mô tả/Layout/Props/Trạng thái theo thiết kế mới, thêm ghi chú không bao gồm badge so sánh kỳ trước.

### Code thay đổi

- `src/components/finance/category-pie-chart.tsx`:
  - Bỏ layout "chart bên trái, legend bên phải"; đổi sang layout dọc: Total Spending (label + số tiền lớn) → Donut Chart (canh giữa) → Legend đầy đủ chiều rộng bên dưới.
  - Bỏ chữ "Tổng chi" hiển thị bên trong lỗ rỗng của donut; lỗ rỗng để trống, tăng `inset` từ 22% lên 26% để vành dày hơn, giống ảnh tham khảo.
  - Legend: mỗi dòng đổi từ 1 dòng (chấm màu + tên + %) sang 2 dòng (tên Category + % ở hàng trên, Amount ở hàng dưới màu nhạt hơn); tăng khoảng cách giữa các dòng (`space-y-lg`) cho thoáng hơn.
  - Không đổi cách tính `color`, `percentage`, `totalSpending` — vẫn dùng nguyên dữ liệu từ `report-service.ts` như cũ.

### Không thay đổi

- `src/features/report/lib/report-service.ts` — không đổi, dữ liệu đầu vào (color, percentage, totalSpending mỗi Category) giữ nguyên như cũ.
- `src/components/finance/weekly-comparison-chart.tsx` — không đổi, Bar Chart giữ nguyên thiết kế hiện tại (Dương chỉ yêu cầu đổi phần Donut Chart).
